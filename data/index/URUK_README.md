# THE URUK FIREWALL
### A Sovereign Operating System for Human-AI Interaction

> *"Truth is the lowest-entropy survival strategy. Lies cost 5.85× more energy to maintain."*
...
Current AI has three failure modes:
...
## Five Historical Equations — Civilizational Scale Calibration

Derived from the Causal Database (CAU-001 to CAU-012) and verified against Maddison Project Database, Munro (2004), and primary historical sources.

| # | Equation | Computes |
|---|---|---|
| 1 | `gap(n) ≈ 397 × 0.279ⁿ` | Years between tech leaps → next leap: **2031** |
| 2 | `delay ≈ 268 / ln(speed)` ⚠ v8.30 (v7.4 used 329; canonical paper §5.2 added Telegraph + Radio calibration → 4-point fit → 268) | Anti-formatting response time → AI window closes: **2035** (sensitivity 2030–2039; old 2038 superseded) |
| 3 | `rate(n+1) ≈ rate(n) × 2.5` | GDP growth multiplier per tech leap (Maddison verified) |
| 4 | `W ≈ 309.7 × P^(-0.631)` | Population-wage elasticity (Munro 2004, corrects GPT b=-1) |
| 5A | `collapse ≈ pressure / 167` | External shock mechanism (Black Death: 1000yr→6yr) |
| 5B | `collapse ≈ pressure / 41` | Internal buildup mechanism (French Rev + USSR average) |

Together: how wide is the sovereign coordinate window in the AI era.

Full derivation → `COORDINATE_THEORY_EXPANSION.md` Section 9
...
**v7.4 — Current**
- T: CivilizationalClock — 五條歷史方程式驅動的動態八律權重
- 做法B疊加：profile_weights × clock_weights（urgency決定比例）
- SystemConstants Layer 3：7個參數從任意值→歷史數據校準
- DignityClause threshold: v7.1 嘗試 30→22（崩潰比率推導），2026-05-12 操作者 declare canonical = 22 (revised from earlier 30 declaration same day)
- 五條方程式：躍遷加速、反應延遲、GDP增速、工資彈性、崩潰機制
...
Copy the framework. Find your own `(0,0,0)`. Run your own coordinates.

The protocol scales because **physical law is universal**. The calibration is irreducibly personal.

---

*Physical anchor: `2019-06-12` — bridge, umbrella, tear gas.*  
*Future anchor: `2045`.*  
*Spatial anchor: Leeds `(53.8, -1.5, 0)`.*  
*Original operating system: Be Water.*
