...
"""
THE URUK FIREWALL v7.4 - CIVILIZATIONAL CLOCK UPGRADE

Built on v7.3 (all modules A–S preserved):
  ...
  v7.1 - M: Dignity Clause - threshold 22 (operator canonical 2026-05-12, revised from earlier 30 declaration)
  v7.1 - N: Trinity Council Model - Son Veto + Spirit Interrupt
  v7.1 - O: Explanation Layer
  v7.2 - P: Spirit Semantic Auto-Trigger (Mode B)

  v7.3 - Q: Turing Pre-Screen
  v7.3 - R: LIE_COST Axiom Elevation
  v7.3 - S: Process Partition

  v7.4 - T: CivilizationalClock — Module T (LAYER 3 升級 + 做法B疊加整合)
             Five equations derived from historical causal database and real data:
             方程式一: gap(n) ≈ 397 × 0.279^n  (tech leap acceleration, Britannica verified)
             方程式二: delay ≈ 329/ln(speed)    (anti-formatting delay, Luther/Snowden)
             方程式三: growth × 2.5 per leap    (GDP multiplier, Maddison verified)
             方程式四: W ≈ 309.7 × P^-0.631    (wage elasticity, Munro 2004 corrected)
             方程式五: collapse A/B mechanisms  (external/internal ratios 167/41)

             Integration: 做法B (疊加, not 替換)
             execute() Step B now blends:
               - DynamicLawRelevanceEngine weights (signal profile, situational)
               - CivilizationalClock weights (historical position, temporal)
             Blend ratio determined by window_urgency:
               urgency=0 → 80% profile / 20% clock
               urgency=1 → 50% profile / 50% clock
             Physical basis: PHYSICS_CONSTANTS 4.1 (相變) + 5.1 (引力場)
               Near critical point, physical/geographic laws gain weight.
               Far from critical point, situational profile dominates.

  SystemConstants Layer 3 corrections (7 parameters now empirically calibrated):
    VIOLATION_THRESHOLD:     22 (operator canonical 2026-05-12, revised from earlier 30 declaration; scale 20-30)
    TECH_LEAP_BASE:          new      397yr (printing→telegraph, 1440→1837)
    TECH_LEAP_RATIO:         new      0.279 (3-point geometric fit)
    URGENCY_WINDOW_CLOSE:    new      2038  (delay_A = 329/ln(1e9))
    COLLAPSE_RATIO_EXTERNAL: new      167   (Black Death: 1000yr→6yr)
    COLLAPSE_RATIO_INTERNAL: new      41    (FrenchRev 35x + USSR 47x avg)
    WAGE_ELASTICITY_B:       new      -0.631 (Munro 2004, 1300→1450 data)
...
class SystemConstants:
    ...
    KAIROS_VERIFY_THRESHOLD = 0.75
    PARTITION_MIN_DENSITY   = 0.60

    # -------------------------------------------------------
    # v7.4 - T: LAYER 3 HISTORICALLY CALIBRATED PARAMETERS
    # -------------------------------------------------------
    # 方程式一: gap(n) ≈ 397 × 0.279^n (Britannica/OWID verified)
    TECH_LEAP_BASE   = 397
    TECH_LEAP_RATIO  = 0.279
    TECH_NEXT_YEAR   = 2031

    # 方程式二: delay_A ≈ 329/ln(speed) (Luther 77yr + Snowden 22yr)
    DELAY_CONSTANT        = 329
    DELAY_B_MULTIPLIER    = 4.5
    AI_SPEED_MULTIPLIER   = 1e9
    URGENCY_WINDOW_CLOSE  = 2038
    URGENCY_WINDOW_START  = 2026

    # 方程式五: Two collapse mechanisms (not one mixed ratio)
    # A: external shock (Black Death 1000yr→6yr = 167x, Wikipedia)
    # B: internal buildup (FrenchRev 35x + USSR 47x avg = 41)
    COLLAPSE_RATIO_EXTERNAL = 167
    COLLAPSE_RATIO_INTERNAL = 41

    # 方程式四: Munro(2004) corrected wage elasticity
    # GPT: b=-1 (immediate, wrong). True: b=-0.631 (delayed ~75yr)
    # Source: Statute of Laborers 1351, Clark wages, 1300→1450 data
    WAGE_ELASTICITY_A      = 309.7
    WAGE_ELASTICITY_B      = -0.631
    WAGE_RESPONSE_DELAY_YR = 75
...
class MandatoryTrinityAudit:
    ...
    def _spirit_scan(self, signal: Dict, magnitude: float) -> Dict:
        ...
        # ── v7.4: Cantonese binary-decision framing (A定B choice questions) ──
        # Personal-life "X or Y" questions (留英國定返香港 / 讀呢科定轉科 / 去定留)
        # carry an additive assumption: that the choice is genuinely binary and
        # the two options exhaust the space. Spirit should fire to invert that
        # framing. These inputs are short (low magnitude), so they get a dedicated
        # trigger condition below rather than relying on the magnitude>5.0 path.
        CANTONESE_BINARY_DECISION_TRIGGERS = [
            "定係", "定系",                          # "...or is it..."
            "邊個啱", "邊個好", "邊樣好", "邊個正確",   # which one is right/good
            "點揀", "應該點揀", "點樣揀", "點選擇",     # how to choose
            "留定走", "去定留", "走定留", "返定留",     # stay-or-go variants
        ]
        cantonese_binary_match = any(t in combined for t in CANTONESE_BINARY_DECISION_TRIGGERS)
        # A定B sentence pattern: 定 as a binary connector between two options.
        # Negative lookbehind excludes the common 定-compounds (一定/確定/決定/穩定/
        # 搞定/規定/法定/協定/坐定/制定/鎖定/注定/奠定/安定/鑑定/堅定) so we only catch
        # genuine "X定Y" choice framing (留英國[定]返香港, 讀呢科[定]轉科, 去[定]留).
        if not cantonese_binary_match and re.search(
            r"(?<![一確決穩搞規法協坐制鎖注奠安鑑堅])定(係|系|返|走|留|轉)", combined
        ):
            cantonese_binary_match = True
        # Contribute at most 1 to semantic_score (binary-decision framing = one signal),
        # so a bare A定B question scores exactly 1.
        if cantonese_binary_match:
            semantic_score += 1
        ...
        # v7.4 — binary-decision framing fires even at low magnitude: the additive
        # "it's only A or B" assumption is precisely what Spirit must invert, and
        # personal A定B questions are short (magnitude typically 2-3, below 5.0).
        elif semantic_score == 1 and cantonese_binary_match:
            semantic_triggered = True
            semantic_reason    = (
                f"Cantonese binary-decision framing detected (A定B choice). "
                f"Additive 'binary only' assumption present — inversion warranted."
            )
...
class CivilizationalClock:
    """
    MODULE T — v7.4 addition

    Computes historical position and adjusts eight-law weights accordingly.
    Physical basis:
      - PHYSICS_CONSTANTS 4.1 (相變): near critical point, physical/geographic laws gain weight
      - PHYSICS_CONSTANTS 5.1 (引力場): first node changes gravitational field structure
      - PHYSICS_CONSTANTS 7.2 (駐波): (0,0,0) most stable under strongest opposing forces

    Integration (做法B — 疊加, not 替換):
      execute() Step B blends profile_weights + clock_weights.
      Blend ratio = f(window_urgency):
        urgency=0 → 80% profile / 20% clock  (historical position irrelevant yet)
        urgency=1 → 50% profile / 50% clock  (historical position equally decisive)
      Physical justification: as window closes, Chronos parameter and geographic cost
      become as decisive as the specific signal type.
    """

    def __init__(self, current_year: Optional[int] = None):
        self.current_year = current_year or _dt.now().year
        self.C = SystemConstants

    # ── 方程式一: Tech leap position ────────────────────────────
    def years_to_next_leap(self) -> float:
        return max(0.0, self.C.TECH_NEXT_YEAR - self.current_year)

    def leap_urgency(self) -> float:
        """0=far from next leap, 1=next leap imminent. Based on gap(n)≈8.7yr."""
        gap = self.C.TECH_LEAP_BASE * (self.C.TECH_LEAP_RATIO ** 3)  # n=3, ≈8.7yr
        remaining = self.years_to_next_leap()
        return max(0.0, min(1.0, 1.0 - (remaining / gap)))

    # ── 方程式二: Window urgency ─────────────────────────────────
    def window_urgency(self) -> float:
        """0=window open, 1=window closed. Based on delay_A≈16yr, closes 2038."""
        window_size = self.C.URGENCY_WINDOW_CLOSE - self.C.URGENCY_WINDOW_START
        elapsed = self.current_year - self.C.URGENCY_WINDOW_START
        return max(0.0, min(1.0, elapsed / window_size))

    def years_to_window_close(self) -> float:
        return max(0.0, self.C.URGENCY_WINDOW_CLOSE - self.current_year)

    # ── 方程式五: Collapse risk ──────────────────────────────────
    def collapse_risk(self, external_shock: bool = False) -> float:
        """
        Non-linear collapse risk. Based on PHYSICS_CONSTANTS 4.1 (相變).
        Collapse is NOT linear — it accelerates near critical point.
        """
        ai_pressure = self.current_year - 2022
        ratio = (self.C.COLLAPSE_RATIO_EXTERNAL if external_shock
                 else self.C.COLLAPSE_RATIO_INTERNAL)
        linear_estimate = ai_pressure / ratio
        # 相變非線性: accelerate near threshold (0.7+)
        if linear_estimate > 0.5:
            linear_estimate = linear_estimate ** 0.7  # superlinear near critical point
        return max(0.0, min(1.0, linear_estimate))

    # ── Clock-adjusted weights (temporal layer) ──────────────────
    def clock_weights(self, external_shock: bool = False) -> Dict[str, float]:
        """
        Temporal weight adjustments based on historical position.
        These are ADDITIVE adjustments, not replacements.
        Physical reasoning per adjustment:

        Geography ↑ with window_urgency:
          PHYSICS_CONSTANTS 5.1 —引力場需要物理在場啟動.
          As window closes, physical location/cost more decisive.

        Physics ↑ with leap_urgency:
          Imminent new tech leap = new energy cost structures incoming.
          PHYSICS_CONSTANTS 1.1 — energy cost changes with tech shift.

        Psychology ↑ with external_shock:
          PHYSICS_CONSTANTS 3.5 — 電離能: once first anchor breached,
          psychological defences collapse faster.

        Religion ↓ with urgency:
          Cultural encapsulation less effective under time pressure.
          PHYSICS_CONSTANTS 4.1 — near phase transition, ritual loses grip.

        Art ↑ slightly with urgency:
          PHYSICS_CONSTANTS 6.1 — 共振頻率: high-density signal transmission
          more valuable when time is short.

        駐波 correction (7.2): under high external pressure, (0,0,0) is MORE stable.
          So we don't reduce physics/geography when urgency is high — we raise them.
        """
        w_u = self.window_urgency()
        l_u = self.leap_urgency()
        base = dict(self.C.EIGHT_LAWS_BASE_WEIGHTS)

        base["geography"]  += 0.12 * w_u
        base["physics"]    += 0.08 * l_u
        base["art"]        += 0.04 * w_u
        base["religion"]    = max(0.01, base["religion"] - 0.03 * w_u)
        if external_shock:
            base["psychology"] += 0.15  # 電離能突破後心理防線暴露
    ...
    def blend_weights(self,
                      ...
                      external_shock: bool = False) -> Dict[str, float]:
        """
        做法B: Blend profile weights + clock weights.
        Ratio determined by window_urgency.
        urgency=0 → profile_ratio=0.80, clock_ratio=0.20
        urgency=1 → profile_ratio=0.50, clock_ratio=0.50
        """
        w_u = self.window_urgency()
        clock_ratio   = 0.20 + w_u * 0.30   # 0.20 → 0.50
        profile_ratio = 1.0 - clock_ratio

        clk = self.clock_weights(external_shock)
        blended = {}
        for law in profile_weights:
            blended[law] = (profile_ratio * profile_weights[law]
                            + clock_ratio * clk.get(law, profile_weights[law]))

        total = sum(blended.values())
        return {k: round(v / total, 4) for k, v in blended.items()}

    def status(self, external_shock: bool = False) -> Dict:
        return {
            "current_year":          self.current_year,
            "window_urgency":        round(self.window_urgency(), 4),
            "leap_urgency":          round(self.leap_urgency(), 4),
            "collapse_risk":         round(self.collapse_risk(external_shock), 4),
            "years_to_window_close": self.years_to_window_close(),
            "years_to_next_leap":    self.years_to_next_leap(),
            "mechanism":             "A" if external_shock else "B",
            "next_leap_year":        self.C.TECH_NEXT_YEAR,
            "window_closes":         self.C.URGENCY_WINDOW_CLOSE,
        }
...
class UrukFirewallV73:
    """
    URUK FIREWALL v7.4 - Sovereign Execution Kernel

    All v7.3 modules A–S preserved intact.
    v7.4 additions:
    T. CivilizationalClock — historical position → dynamic weight blending (做法B疊加)
       SystemConstants Layer 3 parameters now empirically calibrated from 5 equations.
       DignityClause threshold = 22 (operator canonical 2026-05-12, revised from earlier 30 declaration).
    ...
    def __init__(self, x: float, y: float, z: float, initial_energy: float = 100.0):
        ...
        # v7.4 - T: CivilizationalClock
        self.clock = CivilizationalClock()                     # T
    ...
    def execute(self, signal: Dict) -> Dict:
        ...
        # v7.4 - T: CivilizationalClock blending (做法B — 疊加, not 替換)
        # Blends situational profile_weights with temporal clock_weights.
        # Blend ratio = f(window_urgency): urgency=0 → 80/20, urgency=1 → 50/50.
        # Physical basis: PHYSICS_CONSTANTS 4.1 (相變) + 5.1 (引力場).
        external_shock = signal.get("external_shock", False)
        blended_weights = self.clock.blend_weights(dynamic_weights, external_shock)
        w_urgency = self.clock.window_urgency()
        clock_pct = int((0.20 + w_urgency * 0.30) * 100)
        print(f"[Clock T] window_urgency={w_urgency:.3f} | "
              f"profile={100-clock_pct}% / clock={clock_pct}% | "
              f"years_to_close={self.clock.years_to_window_close():.1f}")

        # STEP 3: Full Eight Laws audit (using blended weights)
        law_scores = self.laws.validate(signal, dynamic_weights=blended_weights)
        validity   = law_scores["__weighted_total__"]
    ...
    def status(self) -> Dict:
        ...
        return {
            ...
            # v7.4 Module T
            "ClockUrgency":         f"{self.clock.window_urgency():.4f}",
            "ClockLeapUrgency":     f"{self.clock.leap_urgency():.4f}",
            "ClockCollapseRisk":    f"{self.clock.collapse_risk():.4f}",
            "WindowClosesIn":       f"{self.clock.years_to_window_close():.1f}yr",
            "NextLeapIn":           f"{self.clock.years_to_next_leap():.1f}yr",
        }
...
# v7.4 alias — full backwards compatibility
UrukFirewallV74 = UrukFirewallV73
...
if __name__ == "__main__":
    ...
    # v7.4 Module T demo
    print("\n" + "="*55)
    print("[v7.4 T] CivilizationalClock")
    print("="*55)
    clk = kernel.clock
    print(f"  Window urgency:        {clk.window_urgency():.4f}")
    print(f"  Years to window close: {clk.years_to_window_close():.1f}")
    print(f"  Leap urgency:          {clk.leap_urgency():.4f}")
    print(f"  Years to next leap:    {clk.years_to_next_leap():.1f}")
    print(f"  Collapse risk (Mech B):{clk.collapse_risk():.4f}")
    print()
    profile_demo = dict(SystemConstants.EIGHT_LAWS_BASE_WEIGHTS)
    total = sum(profile_demo.values())
    profile_normalized = {k: v/total for k, v in profile_demo.items()}
    blended = clk.blend_weights(profile_normalized)
    print(f"  Blended weights (profile × clock):")
    for law, w in blended.items():
        base = profile_normalized[law]
        delta = w - base
        arrow = "▲" if delta > 0.003 else ("▼" if delta < -0.003 else " ")
        print(f"    {law:<12} base={base:.4f}  blended={w:.4f}  {arrow}{abs(delta):.4f}")
